...
The configuration file requirements are:
  ...
  - Must defined following variables:

    # The complete Update URL.
    # In the $URL, it's possible to use $WHATSMYIP for the current IP.
    URL=...
...
B<Tip>: to keep track of things, name the configuration files
according to domain names like "mine.duckdns.org.conf"
