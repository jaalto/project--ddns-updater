...
Send status to syslog. Can only be used by root.
...
Program initialization files (read in this order)

  /etc/defaults/ddns-updater
  $HOME/.ddns-updater
...
Configuration file for duckdns.org

  $CONF/duckdns.conf
  DUCKDNS_DOMAINS=host,host...
  DUCKDNS_TOKEN=yourtoken

Configuration file for dns.he.net
